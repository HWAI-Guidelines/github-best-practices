...
for root, dirs, files in os.walk(repo_dir):
    for file in files:
        # Skip README.md, add_license.py, and package.json
        if file in ['README.md', 'add_license.py', 'package.json']:
            continue
        ...
        for ext, comment_style in file_comment_styles.items():
            if file.endswith(ext) or file.lower() == 'dockerfile':
                add_license_to_file(os.path.join(root, file), comment_style)
                break
